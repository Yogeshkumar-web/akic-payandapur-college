...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = db.select().from(studyMaterials).orderBy(desc(studyMaterials.uploadedAt));

    if (classParam) {
      query = db.select().from(studyMaterials).where(eq(studyMaterials.class, classParam)).orderBy(desc(studyMaterials.uploadedAt));
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, fileUrl, class: classValue, subject } = body;
