...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = db.select().from(notices).orderBy(desc(notices.date));

    if (category) {
      query = db.select().from(notices).where(eq(notices.category, category)).orderBy(desc(notices.date));
    }
